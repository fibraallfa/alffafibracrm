...
export async function handleZapiWebhook(request: Request, requiredAgentId?: string) {
  ...
  try {
    ...
    if (requiredAgentId) {
      const agent = await prisma.agent.findFirst({
        where: { id: requiredAgentId, active: true, deletedAt: null },
        select: { zapiInstanceId: true, zapiToken: true, zapiClientToken: true },
      });
      if (!agent?.zapiInstanceId || !agent.zapiToken || !agent.zapiClientToken) {
        return NextResponse.json(errorResponse("Agente indisponivel."), { status: 503 });
      }
      if (payload.instanceId !== agent.zapiInstanceId) {
        return NextResponse.json(errorResponse("Instancia incorreta."), { status: 403 });
      }
    }
