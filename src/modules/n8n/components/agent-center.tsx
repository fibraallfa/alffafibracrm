...
type AgentItem = {
  ...
  zapiBaseUrl: string | null; zapiInstanceId: string | null; zapiToken: string | null;
  zapiWhatsappNumber: string | null; plans: PlanItem[];
};
type Tab = "agents" | "plans" | "openai" | "flow";

const defaultSteps: FlowStep[] = [
  { id: "start", state: "START", title: "Entrada Meta Ads", message: "Olá 👋! Eu sou o Cris, atendente virtual da Claro. Estou aqui pra facilitar seu atendimento. Pode me informar o CEP da instalação?" },
...
function AgentModal({ agent, plans, saving, onClose, onSave }: { agent: AgentItem | null; plans: PlanItem[]; saving: boolean; onClose: () => void; onSave: (data: Record<string, unknown>) => Promise<void> }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const f = new FormData(e.currentTarget); await onSave({
    ...
    zapiBaseUrl: f.get("zapiBaseUrl"), zapiInstanceId: f.get("zapiInstanceId"), zapiToken: f.get("zapiToken"), zapiWhatsappNumber: f.get("zapiWhatsappNumber"),
  ...
  return <Modal title={agent ? `Editar ${agent.name}` : "Cadastrar agente"} onClose={onClose}><form className="space-y-5" onSubmit={submit}>
    ...
    <Section title="Credenciais Z-API"><div className="grid gap-3 sm:grid-cols-2"><Input name="zapiInstanceId" defaultValue={agent?.zapiInstanceId ?? ""} placeholder="Instance ID" /><Input name="zapiToken" defaultValue={agent?.zapiToken ?? ""} type="password" placeholder={agent?.zapiToken ? "Token configurado" : "Token da instância"} /><Input name="zapiWhatsappNumber" defaultValue={agent?.zapiWhatsappNumber ?? ""} placeholder="Número do WhatsApp" /><Input name="zapiBaseUrl" defaultValue={agent?.zapiBaseUrl ?? "https://api.z-api.io"} placeholder="URL base Z-API" /></div></Section>
